...
try:
    ...
    get_mac_for_ipv4 = mac_to_ipv6_linklocal = lambda mac: None
...
def generar_informe_alerta_docx():
    try:
        ...
        p.add_run(time.strftime("%Y-%m-%d %H:%M:%S")) # <--- USA 'time'
        ...
        filename = f"informe_alerta_{alerta_data.get('tipo', 'generico').replace(' ', '_')}_{time.strftime('%Y%m%d%H%M%S')}.docx" # <--- USA 'time'
        ...
        logging.error(traceback.format_exc()) # <--- USA 'traceback'
        return jsonify({"mensaje": "Error interno al generar el informe."}), 500

# No olvides registrar este blueprint en tu app.py principal:
